...
test_that("mod_table_server initializes with reactiveVal DataStore", {
  ...
  testServer(mod_table_server, args = list(store_reactive = store_reactive, store_trigger = store_trigger), {
    expect_equal(output$summary_rows, "254")
    expect_equal(output$summary_cols, "48")
    expect_equal(output$summary_age, sprintf("%.1f", 75.0))
    expect_equal(output$summary_bmibl, sprintf("%.1f", 24.6))
...
test_that("valid table edit updates store and modified count", {
  ...
  testServer(mod_table_server, args = list(store_reactive = store_reactive, store_trigger = store_trigger), {
    ...
    session$setInputs(table_edit = list(row = 0, col = "AGE", value = 30))
    session$setInputs(`_raf_trigger` = 1)
    # expect update applied to store
    expect_equal(as.numeric(store$data[1, "AGE"]), 30)
...
test_that("save flow calls store$save and resets modified count", {
  ...
  testServer(mod_table_server, args = list(store_reactive = store_reactive, store_trigger = store_trigger), {
    # make an edit
    session$setInputs(table_edit = list(row = 1, col = "AGE", value = 22))
    session$setInputs(`_raf_trigger` = 1)
    expect_equal(store$get_modified_count(), 1)
    ...
    expect_equal(store$original$AGE[1], store$data$AGE[1])
  })
})
...
test_that("revert flow calls store$revert and resets modified count", {
  ...
  testServer(mod_table_server, args = list(store_reactive = store_reactive, store_trigger = store_trigger), {
    session$setInputs(table_edit = list(row = 1, col = "AGE", value = 10))
    session$setInputs(`_raf_trigger` = 1)
    expect_equal(store$get_modified_count(), 1)
    ...
    expect_equal(store$get_modified_count(), 0)
    expect_equal(store$data$AGE[1], store$original$AGE[1])
  })
})
